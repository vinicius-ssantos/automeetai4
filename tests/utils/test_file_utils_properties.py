...
@settings(deadline=None)  # Disable deadline to allow for longer execution times with complex Unicode paths
...
def test_validate_file_path_with_allowed_extensions(file_path, extensions):
    ...
    """
    # Skip test if the path contains a suspicious pattern
    # This is necessary because the validate_file_path function now rejects paths with suspicious patterns
    # even if they were generated by the valid_file_path strategy
    if any(pattern in file_path for pattern in suspicious_patterns):
        pytest.skip(f"Skipping test for path with suspicious pattern: {file_path}")
