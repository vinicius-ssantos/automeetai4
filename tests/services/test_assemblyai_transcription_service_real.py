...
@pytest.mark.manual
class TestAssemblyAITranscriptionServiceReal:
    """
    Tests for the AssemblyAITranscriptionService using the real API.
    
    These tests call the real AssemblyAI API and consume credits.
    They are marked with @pytest.mark.manual and will be skipped by default.
    To run these tests, use: pytest tests/test_assemblyai_transcription_service_real.py --manual
    """
    ...
    def test_transcribe_real_audio(self):
        """Test transcribing a real audio file using the AssemblyAI API."""
        ...
        # If there are utterances, print the first one
        if result.utterances and len(result.utterances) > 0:
            print(f"First utterance: {result.utterances[0].text[:100]}...")